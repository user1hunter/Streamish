...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        [Fact]
        public void Search_Returns_Correct_Results_By_Criterion()
        {
            string searchText = "testText";
            string exclusionText = "exclusion";
            var videoIdWithSearchText = 5;
            var videos = CreateTestVideos(10);
            foreach (var v in videos)
            {
                v.Title = exclusionText;
                v.Description = exclusionText;
            }
            videos[0].Title = searchText;
            videos[0].Id = videoIdWithSearchText;


            var repo = new InMemoryVideoRepository(videos);
            var controller = new VideoController(repo);

            var result = controller.Search(searchText, false);

            var okResult = Assert.IsType<OkObjectResult>(result);
            var actualVideos = Assert.IsType<List<Video>>(okResult.Value);

            var videoFromDbThatMatches = actualVideos.FirstOrDefault(v => v.Id == videoIdWithSearchText);

            Assert.NotEmpty(actualVideos);
            Assert.Single(actualVideos);
            Assert.NotNull(videoFromDbThatMatches);


        }

        [Fact]
        public void Search_Returns_Descending_Results()
        {
            bool sortDesc = true;
            int numOfVideos = 10;
            var videos = CreateTestVideos(numOfVideos);
            var sortedVideos = videos.OrderByDescending(v => v.DateCreated);

            var repo = new InMemoryVideoRepository(videos);
            var controller = new VideoController(repo);

            var result = controller.Search("", sortDesc);

            var okResult = Assert.IsType<OkObjectResult>(result);
            var actualVideos = Assert.IsType<List<Video>>(okResult.Value);

            Assert.NotEmpty(actualVideos);
            Assert.Equal(numOfVideos, actualVideos.Count);
            Assert.Equal(sortedVideos, actualVideos);
