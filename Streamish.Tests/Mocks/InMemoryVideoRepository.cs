...
namespace Streamish.Tests.Mocks
{
    class InMemoryVideoRepository : IVideoRepository
    {
        ...
        public List<Video> Search(string criterion, bool sortDescending)
        {
            ...
            var videosByCriterion = _data.Where(v => v.Title.Contains(criterion) || v.Description.Contains(criterion)).ToList();

            if (sortDescending)
            {
                videosByCriterion = videosByCriterion.OrderByDescending(v => v.DateCreated).ToList();
            }
            else
            {
                videosByCriterion = videosByCriterion.OrderBy(v => v.DateCreated).ToList();
            }

            return videosByCriterion;
        }
